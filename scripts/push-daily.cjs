...
function formatContent(content) {
  ...
  let bullets = lines.filter(l => l.startsWith("•") || l.startsWith("-"));
  let text = lines.filter(l => !l.startsWith("•") && !l.startsWith("-"));
  let result = text.slice(0, 10).join("\n");  // show text
  if (bullets.length > 0) {
    result += "\n\n";
    result += bullets.map(b => b.replace(/^[•\-]\s*/, "▸ ")).join("\n");
      ...
      const feishuMsg = {
        ...
        card: {
          ...
          elements: [
            ...
            { tag: "hr" },
            { tag: "markdown", content: `**🔍 知识要点**\n${body}` },
            { tag: "hr" },
            { tag: "markdown", content: `📖 ${c.source || ""}` },
